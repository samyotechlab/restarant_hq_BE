...
async def _populate_campaign(doc: dict, db) -> OfferResponse:
    """
...
class OfferController:
    ...
    @staticmethod
    async def create_offer(data: OfferCreate, db) -> OfferResponse:
        ...
        offer_doc = {
            ...
            "status": data.status.value if data.status else OfferStatus.SCHEDULED.value,
        ...
        result = await db["offer"].insert_one(offer_doc)
        offer_doc["_id"] = result.inserted_id
        return await _populate_campaign(offer_doc, db)

    @staticmethod
    async def get_offer(offer_id: str, db) -> OfferResponse:
        """Fetch a single campaign by ID — menu items are populated."""
        _validate_object_id(offer_id, "campaign ID")

        offer = await db["offer"].find_one({"_id": ObjectId(offer_id)})
        if not offer:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Offer not found.",
            )
        return await _populate_campaign(offer, db)

    @staticmethod
    async def get_all_offers(db) -> list[OfferResponse]:
        """Return all campaigns unpaginated."""
        offers = await db["offer"].find().to_list(length=None)
        return [await _populate_campaign(c, db) for c in offers]

    @staticmethod
    async def get_paginated_offers(db, page: int = 1, limit: int = 10) -> PaginatedOfferResponse:
        """Return a paginated list of campaigns."""
        skip = (page - 1) * limit

        total_results = await db["offer"].count_documents({})
        offers = await db["offer"].find().skip(skip).limit(limit).sort('created_at', -1).to_list(length=None)
        ...
        return PaginatedOfferResponse(
            ...
            data=[await _populate_campaign(c, db) for c in offers],
        )

    @staticmethod
    async def update_offer(offer_id: str, data: OfferUpdate, db) -> OfferResponse:
        """Partial update — only the fields provided in the request body are changed."""
        _validate_object_id(offer_id, "campaign ID")

        existing = await db["offer"].find_one({"_id": ObjectId(offer_id)})
        if not existing:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="offer not found.",
        ...
        await db["offer"].update_one(
            {"_id": ObjectId(offer_id)},
            {"$set": update_fields},
        )

        updated = await db["offer"].find_one({"_id": ObjectId(offer_id)})
        return await _populate_campaign(updated, db)

    @staticmethod
    async def delete_offer(offer_id: str, db) -> dict:
        """Hard-delete a campaign."""
        _validate_object_id(offer_id, "offer ID")

        result = await db["offer"].delete_one({"_id": ObjectId(offer_id)})
        if result.deleted_count == 0:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Offer not found.",
            )
        return {"deleted_id": offer_id}